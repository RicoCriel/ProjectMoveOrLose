...
public class MapGeneratorOld : MonoBehaviourPunCallbacks
{
    public static MapGeneratorOld instance; // Singleton instance of MapGenerator
    ...
    void DamageBlockRPC(Vector3 pos, int damage)
    {
        ...
        if (foundBlock != null)
        {
            ...
            if (foundBlock.TakeDamageAndCheckIfDead(damage))
            {
                ...
                MapGeneratorOld.instance.EditMapState(intPos, 0);
            }
            else
            {
                MapGeneratorOld.instance.EditMapState(intPos, foundBlock.GetCurrentHealth());
            }
        }
    }
